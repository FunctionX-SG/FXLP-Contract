...
describe("UpdateFeeTreasury", () => {
  ...
  it("Authorized user", async () => {
    const { stFX, authorizedUser, notAuthorizedUsers } =
      await loadContractFixture();

    for (const notAuthorizedUser of notAuthorizedUsers) {
      await stFX
        .connect(authorizedUser)
        .callStatic.updateFeeTreasury(notAuthorizedUser.address);
    }
  });
  it("Not authorized user", async () => {
    ...
    for (let i = 0; i < notAuthorizedUsers.length; i++) {
      ...
      await expect(
        stFX
          .connect(notAuthUser)
          .callStatic.updateFeeTreasury(notAuthorizedUsers[value].address)
